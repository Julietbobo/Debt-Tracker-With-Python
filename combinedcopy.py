...
def dashboard_page():
    ...
    try:
        ...
        cur.execute("SELECT SUM(paid_amount) FROM debts WHERE unpaid_amount > 0")
        total_paid = cur.fetchone()[0] or 0

        # Number of current debtors (unpaid > 0)
        cur.execute("SELECT COUNT(DISTINCT customer_name) FROM debts WHERE unpaid_amount > 0")
...
def edit_debtor_page():
    ...
    try:
        ...
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT DISTINCT customer_name FROM debts WHERE unpaid_amount > 0")
