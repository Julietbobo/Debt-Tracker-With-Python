...
def registration_page():
    ...
    cleanphone=phone.strip().replace(" ","").replace("-","")
    validphone=re.fullmatch(r"^(?:\+254|254|0)?[7][0-9]{8}$",cleanphone)
    if phone and not validphone:
     st.warning("Please enter a valid kenyan phone number")

    password = st.text_input("Password", type="password", key="reg_password")
    confirm_password = st.text_input("Confirm Password", type="password", key="reg_confirm")
    pattern = r"^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d@$!%*#?&]{7,}$"

    if password and confirm_password:
     if password != confirm_password:
         st.error("Passwords do not match.")
     elif not re.fullmatch(pattern, password):
        st.error("Password must be at least 7 characters, include at least one letter and one number.")
     else:
    ...
    if st.button("Register"):
        ...
        if not first_name.strip():
            st.warning("First Name can't be empty")
            return
        if not last_name.strip():
            st.warning("Last Name can't be empty")
            return
